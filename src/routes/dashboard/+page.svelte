...
<div class="flex items-start justify-between gap-4">
  <div class="space-y-1">
    <h1 class="text-xl font-semibold tracking-tight">菜单管理后台</h1>
    <p class="text-sm text-muted-foreground">
      管理菜单中的菜品信息，包括添加、编辑、删除菜品，以及设置分类和折扣
    </p>
  </div>
</div>
      ...
      >
        ...
        <div class="flex gap-1">
          <Button.Root
            ...
            size="xs"
            onclick={() => (filterCategory = "all")}>全部</Button.Root
          >
  ...
  <Card.Content class="pt-4">
    ...
    {#if filteredDishes.length === 0}
      <div class="py-24 text-center">
        <div
          class="w-16 h-16 bg-muted rounded-2xl flex items-center justify-center mx-auto mb-4"
        >
      ...
      <div class="overflow-x-auto -mx-6">
        <table class="w-full">
          <thead>
            <tr class="border-b">
              <th
                class="text-left text-xs font-medium text-muted-foreground px-4 py-2"
                >菜品</th
              >
              <th
                class="text-left text-xs font-medium text-muted-foreground px-4 py-2"
              ...
              >
              <th
                class="text-right text-xs font-medium text-muted-foreground px-4 py-2"
                >操作</th
              >
            </tr>
          </thead>
